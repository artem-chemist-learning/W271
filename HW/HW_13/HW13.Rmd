...
acf_plot <- acf(decomp_df$detrended, plot = F,lag.max = 50) %>% autoplot() + xlim(1,50)
pacf_plot <- pacf(decomp_df$detrended, plot = F,lag.max = 50) %>% autoplot() + xlim(1,50)
acf_plot | pacf_plot
```
```{r Manual Model estimation}
model.manual<- arima(monthly_sales$sales, order = c(1, 1, 4))
checkresiduals(model.manual)
```


```{r auto model estimation}
model.auto <- monthly_sales$sales %>% ts(frequency = 12) %>% auto.arima(d = 1, D = 0,
...
```{r auto model estimation}
model.noseason <- monthly_sales$sales %>% auto.arima(d = 1,
