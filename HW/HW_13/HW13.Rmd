...
isales$inv_date <- as.Date(isales$inv_date)
isales$total <- as.numeric(isales$total)

#get a list of customers, who bought more than 1M worth of product in 10 years
big_customers <- isales %>% group_by(cus_name)%>% summarise(rev = sum(total)) %>% filter(rev > 500000)

# Get sales for big customers only
sales <- isales %>% filter(cus_name %in% big_customers$cus_name)
```
...
monthly_sales <- isales %>% group_by(inv_date) %>% summarize(sales = sum(total))
monthly_sales <- monthly_sales %>% as_tsibble(index = inv_date)
#Plot the original seris


monthly_sales %>% 
  ggplot() + aes(x = inv_date, y = sales) + geom_line() 
