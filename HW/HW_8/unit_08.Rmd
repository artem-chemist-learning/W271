...
> After de-seasoning, the timeseries becomes more stationary, with mean around 0 and standard deviation without much change. Characteristic decay of ACF fanction indicates an AR process, which is consistent with quick decay of PACF plot. From these graphs it apperas that it might be an AR(3) process, becasue 3'd lag is the last one strongly significant on PACF plot. To confirm these observations we will perform a grid search using BIC to evaluate goodness of fit.
...
It was not clear what would cause the seasonality, but running a few experiemtns we can find that de-trending with lag around half a year significantly reduces these artefacts. This comes as  no rurprise after findings in Montana data.
