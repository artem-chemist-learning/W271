...
arima_seasonal <- arima(covid_MT$diff, order = c(3,0,0), seasonal = c(0, 0, 0), include.mean = T)
plot_seasonal <- arima_seasonal %>% residuals() %>% acf(plot = FALSE, na.action = na.pass) %>% autoplot() + 
...
```
``` {r Ljung-Box test}
resid.ar3 <- as.ts(arima_seasonal %>% residuals())
...
covid_all_df <- covid_df[columns_to_select]

covid_all_df$update_date <- as.Date(covid_all_df$date_updated)
covid_all_ts <- aggregate(covid_all_df["new_cases"], by=covid_all_df["update_date"], sum)
covid_all_ts <- as_tsibble(covid_all_ts, index = update_date)
...
covid_overall<- mutate(covid_all_ts, diff = difference(new_cases, lag=24))

ts_plot <- covid_overall %>% ggplot() + aes(x = update_date, y = new_cases) + geom_line()

ts_diff_plot <- covid_overall %>% ggplot() + aes(x = update_date, y = diff) + geom_line()

acf_plot<-covid_overall %>% 
  ACF(diff, type="correlation") %>% autoplot()  + labs (x = "lag", y = "ACF")

pacf_plot<-covid_overall %>% 
  ACF(diff, type="partial") %>% autoplot() + labs (x = "lag", y = "PACF")
