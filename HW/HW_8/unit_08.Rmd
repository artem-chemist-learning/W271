...
```
> Focusing on detrended series and removing once-in-a-lifetime anomalie, we can see that ACF plot decays quickly, loosing signoficance after lag 11. This is indicative of an MA process. At the same time, PACF plot is consistently significant up to lag 6. Pike at lag 12 may or may not be significant.
...
> The selected model, therefore, is ARIMA (1,1,1) with coefficients autoregressive coefficient -0.7277, movina average coefficeint 0.8019 and a constant term (average) of -0.0018. I.e. after one level of differencing, in th eresulting autoregressive model the predicted value will depend on the lag 1 value with coeff -0.7277 and on the lag error with coeff -0.0018
...
# Obtain residuals plot for selected model
dat <- unemployment %>% filter(name == "overall")
...
#par(mfrow=c(3,1))
plot(covid_MT$tot_cases, xlab="t", ylab="Xt", col="cornflowerblue", main="Time Series")
acf(covid_MT$tot_cases, col="darkorange2", main="ACF")
pacf(covid_MT$tot_cases, col="gold3", main="PACF")
covid_MT %>% gg_lag(tot_cases, lag = 1:9, geom = "point", color = 1)
```
```{r ARIMA model selection}

model.bic<-covid_MT %>%
model(ARIMA(tot_cases ~ 1 + pdq(1:10,1:2,1:10) + PDQ(0,0,0), ic="bic", stepwise=F, greedy=F))
model.bic %>%
report()
```
...
autoplot()
```
> The residuals do not look like random white noise. There appers to be a pattern where residuals from the same year tend to  have the same sign, i.e. yearly pattern. More complex model might be able to improve fit.

```{r}

model_seasonal<-covid_MT %>%
model(ARIMA(tot_cases ~ 1 + pdq(1:10,1:2,1:10) + PDQ(0:10, 0:2, 0:10, period = "1 month"), ic="bic", stepwise=F, greedy=F))
